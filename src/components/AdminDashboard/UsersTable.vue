  ...
  <div class="bg-white dark:bg-[#1f2937] dark:text-gray-100 shadow-lg rounded-2xl p-6">
    ...
    <div class="flex justify-between items-center mb-6">
      <h2 class="text-2xl font-semibold text-[#5984C6] dark:text-[#8db4ff]">
        {{ texts[lang].adminDashboard.users.title }}
      </h2>
        ...
        <thead class="bg-[#5984C6] text-white">
          <tr>
            <th class="py-3 px-4 text-left">
              {{ texts[lang].adminDashboard.users.profile }}
            </th>

            <th class="py-3 px-4 text-left">
              {{ texts[lang].adminDashboard.users.name }}
            </th>

            <th class="py-3 px-4 text-left">
              {{ texts[lang].adminDashboard.users.phone }}
            </th>

            <th class="py-3 px-4 text-left">
              {{ texts[lang].adminDashboard.users.address }}
            </th>

            <th class="py-3 px-12 text-left">
              {{ texts[lang].adminDashboard.users.email }}
            </th>

            <th class="py-3 px-4 text-left">
              {{ texts[lang].adminDashboard.users.status }}
            </th>

            <th class="py-3 px-4 text-left">
              <button
                @click="toggleSort('createdAt')"
                class="flex items-center gap-1 hover:bg-gray-100 dark:hover:bg-gray-700 px-2 py-1 rounded"
              >
                {{ texts[lang].adminDashboard.users.created }}
                <svg
            ...
            <th class="py-3 px-8 text-left">
              {{ texts[lang].adminDashboard.users.actions }}
            </th>
          </tr>
        </thead>
          ...
          >
            <td class="py-3 px-6">
              <div class="flex items-center gap-3">
                <div class="h-10 w-10 rounded-full overflow-hidden bg-[#e8f0fe] dark:bg-gray-700">
                  <img
            ...
            <td class="py-3 px-6">
              {{ client.name || texts[lang].adminDashboard.users.noName }}
            </td>
            ...
            <td class="py-3 px-3 flex items-center gap-2">
              ...
              <button
                ...
                :title="texts[lang].adminDashboard.users.view"
              >
                <i class="bi bi-eye"></i>
              </button>
              ...
              <button
                ...
                :title="texts[lang].adminDashboard.users.suspend"
              >
              ...
              <button
                ...
                :title="texts[lang].adminDashboard.users.reactivate"
              >
              ...
              <button
                ...
                :title="texts[lang].adminDashboard.users.delete"
              >
    ...
    >
      ...
      >
        <h3 class="text-xl font-semibold text-red-600 mb-4">
          {{ texts[lang].adminDashboard.users.deleteUser }}
        </h3>

        <p class="text-gray-600 dark:text-gray-300 mb-6">
          {{ texts[lang].adminDashboard.users.confirmDelete }}
        ...
        <div class="flex justify-center space-x-4">
          ...
          >
            {{ texts[lang].adminDashboard.users.delete }}
          </button>
          ...
          >
            {{ texts[lang].adminDashboard.users.cancel }}
          </button>
        </div>
      </div>
      ...
      >
        <h3 class="text-xl font-semibold text-red-600 mb-4">
          {{ texts[lang].adminDashboard.users.suspendUser }}
        </h3>

        <p class="text-gray-600 dark:text-gray-300 mb-6">
          {{ texts[lang].adminDashboard.users.confirmSuspend }}
          "<strong>{{ selectedClient?.name }}</strong>"?
        </p>
        ...
        <div class="flex justify-center space-x-4">
          <button
            ...
            class="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700"
          >
            {{ texts[lang].adminDashboard.users.suspend }}
          </button>
      ...
      >
        <h3 class="text-xl font-semibold text-green-600 mb-4">
          {{ texts[lang].adminDashboard.users.reactivateUser }}
        </h3>

        <p class="text-gray-600 dark:text-gray-300 mb-6">
          {{ texts[lang].adminDashboard.users.confirmReactivate }}
          "<strong>{{ selectedClient?.name }}</strong>"?
        </p>
        ...
        <div class="flex justify-center space-x-4">
          ...
          >
            {{ texts[lang].adminDashboard.users.reactivate }}
          </button>
      ...
      >
        <div class="flex items-center gap-4 mb-4">
          <div class="h-14 w-14 rounded-full bg-[#e8f0fe] dark:bg-gray-800 overflow-hidden">
            <img
              :src="getUserImage(selectedClient) || defaultAvatar"
              alt="avatar"
              class="h-full w-full object-cover"
              @error="onUserImgError"
            />
          ...
          <div class="flex-1 min-w-0">
            <h3 class="text-2xl font-semibold text-[#5984C6] truncate">
              {{ selectedClient?.name || texts[lang].adminDashboard.users.client }}
            </h3>

            <span
              class="px-2 py-1 rounded-full text-xs font-semibold mt-1 inline-block"
              :class="{
                'bg-red-100 text-red-600': selectedClient?.status === 'banned',
                'bg-yellow-100 text-yellow-600': selectedClient?.status === 'pending',
                'bg-green-100 text-green-600': selectedClient?.status === 'active',
              }"
            >
              {{ texts[lang].adminDashboard.users[selectedClient?.status || 'active'] }}
            </span>

            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400 truncate">
              <i class="bi bi-geo-alt-fill mr-1"></i>{{ formatClientAddress(selectedClient) }}
            </p>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
          ...
          <div class="sm:col-span-2">
            ...
            <p class="font-medium">{{ formatClientAddress(selectedClient) }}</p>
          </div>
          ...
          <div>
            ...
            <p class="font-medium">
              {{ selectedClient?.createdAt ? new Date(selectedClient.createdAt).toLocaleString() : '-' }}
            </p>
          </div>
        </div>
        ...
        <div class="flex justify-end mt-6">
          <button
            ...
            class="px-4 py-2 bg-gray-200 dark:bg-gray-700 dark:text-gray-100 rounded-md hover:bg-gray-300 dark:hover:bg-gray-600"
          >
            {{ texts[lang].adminDashboard.users.close }}
          </button>
        </div>
      </div>
    </div>
...
const getUserImage = (u) =>
  u?.profileImage || u?.photoURL || u?.avatar || u?.image || "";

const onUserImgError = (e) => {
